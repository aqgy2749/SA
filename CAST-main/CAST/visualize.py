...
def kmeans_plot_multiple(
    ...
    cluster_pl = sns.color_palette('tab20', k)

    print('Plotting the KMeans clustering results...')
    cell_label_idx = 0
    cell_label_dict = {}
    ...
    if plot_strategy == 'sep':
        ...
        for j in range(num_plot):
            ...
            cell_type_t = cell_label[cell_label_idx:(cell_label_idx + coords0.shape[0])].copy()
            cell_label_dict[graph_list[j]] = cell_type_t
            ...
            plt.title(str(graph_list[j]) + ' (KMeans, k = ' + str(k) + ')', fontsize=20)
    ...
    if save_label_dict:
        np.savez(
            os.path.join(output_path_t, f'{taskname_t}_kmeans_labels_k{str(k)}.npz'),
            **{str(sample): labels for sample, labels in cell_label_dict.items()}
        )

    if return_label_dict:
        return cell_label, cell_label_dict
    return cell_label
