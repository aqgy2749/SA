...
#     ### panel 2a：左右分开显示 query / reference ###
#     plt.figure(figsize=[20,12])
#     plt.subplot(1,2,1)
#     plt.rcParams.update({'font.size' : 10,'axes.titlesize' : 20,'pdf.fonttype':42})
#     col = coords_q[idx_q,0]
...
#     col_q = coords_q[idx_q,0]
#     row_q = coords_q[idx_q,1]
#     cell_type_q = cell_label[0:coords_q[idx_q].shape[0]]
#     for i in set(cell_type_q):
#         plt.scatter(np.array(col_q)[cell_type_q == i],
#                     np.array(row_q)[cell_type_q == i],
#                     s=12,edgecolors='none',alpha = 0.5,rasterized=True,
#                     c=np.array(cluster_pl)[cell_type_q[cell_type_q == i]],label = f'Q-{i}')

#     col_r = coords_r[idx_r,0]
#     row_r = coords_r[idx_r,1]
#     cell_type_r = cell_label[coords_q[idx_q].shape[0]:]
#     for i in set(cell_type_r):
#         plt.scatter(np.array(col_r)[cell_type_r == i],
#                     np.array(row_r)[cell_type_r == i],
#                     s=12,edgecolors='none',alpha = 0.5,rasterized=True,
#                     c=np.array(cluster_pl)[cell_type_r[cell_type_r == i]],label = f'R-{i}')

#     plt.axis('equal')
#     plt.xticks(fontsize=20)
#     plt.yticks(fontsize=20)
#     plt.title('Overlay (Query + Reference, KMeans k = ' + str(k) + ')',fontsize=30)
#     add_scale_bar(200,'200 µm')
#     plt.savefig(f'{output_path}/{prefix}_Results_2_overlap.pdf',dpi = 300)
...
def register_result(
    ...
    coords_q_plot = coords_q[idx_q]
    coords_r_plot = coords_r[idx_r]

    if (cell_label_q is not None) and (cell_label_r is not None):
        plot_label_q = np.asarray(cell_label_q)[idx_q]
        plot_label_r = np.asarray(cell_label_r)[idx_r]
        if cluster_pl is None:
            palette_size = int(k) if k is not None else int(max(plot_label_q.max(), plot_label_r.max()) + 1)
            cluster_pl = sns.color_palette('tab20', palette_size)
    else:
        if embed_stack is None:
            raise ValueError('embed_stack cannot be None when external cell labels are not provided.')
        kmeans = KMeans(n_clusters=k, random_state=0).fit(embed_stack)
        cell_label = kmeans.labels_
        n_q = coords_q_plot.shape[0]
        plot_label_q = cell_label[:n_q]
        plot_label_r = cell_label[n_q:(n_q + coords_r_plot.shape[0])]
        if cluster_pl is None:
            palette_size = int(max(plot_label_q.max(), plot_label_r.max()) + 1)
            cluster_pl = sns.color_palette('tab20', palette_size)

    ### panel 1 ###
    plot_mid(coords_q_plot, coords_r_plot, output_path, f'{prefix}_Results_1', scale_bar_t=None)
    ...
    col = coords_q_plot[:, 0]
    row = coords_q_plot[:, 1]
    for i in set(plot_label_q):
        plt.scatter(
            np.array(col)[plot_label_q == i],
            np.array(row)[plot_label_q == i],
            s=12,
            edgecolors='none',
            alpha=0.5,
            rasterized=True,
            c=np.array(cluster_pl)[plot_label_q[plot_label_q == i]],
            label=str(i),
        )
    plt.axis('equal')
    plt.xticks(fontsize=20)
    ...
    col = coords_r_plot[:, 0]
    row = coords_r_plot[:, 1]
    for i in set(plot_label_r):
        plt.scatter(
            np.array(col)[plot_label_r == i],
            np.array(row)[plot_label_r == i],
            s=12,
            edgecolors='none',
            alpha=0.5,
            rasterized=True,
            c=np.array(cluster_pl)[plot_label_r[plot_label_r == i]],
            label=str(i),
        )
    plt.axis('equal')
    plt.xticks(fontsize=20)
    ...
    plt.savefig(f'{output_path}/{prefix}_Results_2.pdf', dpi=300)
    plt.savefig(f'{output_path}/{prefix}_Results_2_merge.pdf', dpi=300)

    ### panel 2b：overlay 图 ###
    plt.figure(figsize=[10, 12])
    plt.rcParams.update({'font.size': 10, 'axes.titlesize': 20, 'pdf.fonttype': 42})

    col_q = coords_q_plot[:, 0]
    row_q = coords_q_plot[:, 1]
    for i in set(plot_label_q):
        plt.scatter(
            np.array(col_q)[plot_label_q == i],
            np.array(row_q)[plot_label_q == i],
            s=12,
            edgecolors='none',
            alpha=0.5,
            rasterized=True,
    ...
    col_r = coords_r_plot[:, 0]
    row_r = coords_r_plot[:, 1]
    for i in set(plot_label_r):
        plt.scatter(
            np.array(col_r)[plot_label_r == i],
            np.array(row_r)[plot_label_r == i],
            s=12,
            edgecolors='none',
            alpha=0.5,
            rasterized=True,
    ...
    plt.figure(figsize=[20, 12])
    plt.subplot(1, 2, 1)
    t_score = J_cal(torch.from_numpy(coords_q_plot), torch.from_numpy(coords_r_plot), cov_anchor_t, bleeding)
    plt.scatter(
        coords_q_plot[:, 0],
        coords_q_plot[:, 1],
        c=1 - t_score,
    ...
    add_scale_bar(200, '200 µm')
    plt.subplot(1, 2, 2)
    plt.scatter(coords_q_plot[0, 0], coords_q_plot[0, 1], c=[0], cmap='vlag', vmin=-1, vmax=1, s=15, alpha=0.5)
    plt.axis('off')
    plt.colorbar()
    plt.savefig(f'{output_path}/{prefix}_Results_3.pdf', dpi=300)
