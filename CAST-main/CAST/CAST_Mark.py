...
def delaunay_dgl(sample_name, df, output_path,if_plot=True,strategy_t = 'convex'):
    ...
    g = dgl.from_networkx(delaunay_graph)
    g = dgl.remove_self_loop(g)
    g = dgl.add_self_loop(g)

    print(f"{sample_name}: coords={coords.shape[0]}, nx_nodes={delaunay_graph.number_of_nodes()}, dgl_nodes={g.number_of_nodes()}")
    print(f"{sample_name}: zero_in_degree_nodes={(g.in_degrees() == 0).sum().item()}")
