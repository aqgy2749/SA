...
def CAST_STACK(
    ...
    def _get_global_labels(label_dict, sample_key):
        if label_dict is None:
            return None
        if sample_key in label_dict:
            return label_dict[sample_key]
        sample_key_str = str(sample_key)
        if sample_key_str in label_dict:
            return label_dict[sample_key_str]
        raise KeyError(f'Cannot find global KMeans labels for sample: {sample_key}')
    ...
    # Plot initial coordinates
    if mid_visual and (global_label_dict is None):
    ...
    # default affine checkpoint path
    ...
    # optionally load affine checkpoint and skip prelocate + Affine_GD
    affine_loaded = False
    if load_affine_ckpt and os.path.exists(affine_ckpt_path):
        ...
        if isinstance(ckpt, dict) and ('theta_r1' in ckpt) and ('theta_r2' in ckpt):
            ...
            if strict_affine_ckpt_match and (ckpt_q is not None) and (ckpt_r is not None):
                if (ckpt_q == query_sample) and (ckpt_r == ref_sample):
    ...
    if not affine_loaded:
        ...
        params_dist.theta_r1 = theta_r1_t
        coords_query_r1 = affine_trans_t(params_dist.theta_r1, coords_query)
        plot_mid(coords_query_r1.cpu(), coords_ref.cpu(), output_path, prefix_t + '_prelocation') if mid_visual else None
    ...
    label_q = _get_global_labels(global_label_dict, query_sample)
    label_r = _get_global_labels(global_label_dict, ref_sample)
    plot_k = global_k if global_k is not None else 20
    ...
    register_result(
        ...
        max_minus_value_t(corr_q_r).cpu(),
        params_dist.bleeding,
        None if global_label_dict is not None else embed_stack_t,
        output_path,
        k=plot_k,
        prefix=prefix_t,
        scale_t=1,
        index_list=[sub_node_idxs[k_t] for k_t in graph_list],
        cell_label_q=label_q,
        cell_label_r=label_r,
    )

    if params_dist.iterations_bs[round_t] != 0:
        ### B-Spline free-form deformation
        padding_rate = params_dist.PaddingRate_bs
        ...
        setattr(params_dist, 'img_size_bs', [(max_xy_tmp * (1 + padding_rate * 2)).cpu()])
        params_dist.min_qr2 = [adj_min_qr2]
        t1 = BSpline_GD(
