...
output_path = f'{work_dir}/output_cast4_pca_2'      ############
...
#     print(adata.X[:5])
#     print(np.min(adata.X), np.max(adata.X))
#     OUTPUT_H5AD = f'{output_path}/demo1_cast4.h5ad'                    ########################
#     adata.write_h5ad(OUTPUT_H5AD)
#     print(f"\n✅ 成功保存 AnnData 文件到 {OUTPUT_H5AD}")

# ###########################PCA(替代CAST.MARK)##########################################
# adata = ad.read_h5ad(f'{output_path}/demo1_cast4.h5ad')                #########################
# adata.layers['norm_1e4'] = adata.X

# # === 用 PCA 直接得到每个 spot 的低维表示，替代 CAST_MARK 的图嵌入 ===
# # 建议在所有样本拼接后 fit PCA，保证不同样本在同一 PCA 基底下可比。
# PCA_N_COMPONENTS = 100  # 你之前 CAST_MARK encoder_dim=100，这里保持一致
# X_all = adata.layers['norm_1e4']
# try:
#     X_all = X_all.toarray()  # 若是稀疏矩阵
# except Exception:
#     X_all = X_all
...
# # embed_dict 结构必须与 CAST_MARK 输出一致：{sample: torch.Tensor [n_spots, d]}
# embed_dict = {
#     sample_t: torch.tensor(
#         X_pca[np.array(adata.obs['sample'] == sample_t)],
#         dtype=torch.float32
#     )
#     for sample_t in samples
# }

# # 可选：保存 PCA embedding，方便复现
# torch.save(embed_dict, f'{output_path}/demo_embed_dict_pca.pt')

# # 可选：看一下 PCA embedding 的聚类可视化
# kmeans_plot_multiple(embed_dict, samples, coords_raw, 'demo1_pca', output_path, k=20, dot_size=10, minibatch=False)
...
#     anchor = coords_raw[s][0].copy()   
#     coords_raw[s] = coords_raw[s] - anchor
embed_dict = torch.load(f'{output_path}/demo_embed_dict_pca.pt',map_location='cpu')#来自cast4_pca
# graph_list = ['15','0']
...
#     coords_raw, embed_dict, output_path, graph_list,
#     params_dist=params_dist,          # 你可以换一套Spline参数
#     load_affine_ckpt=True,
#     save_affine_ckpt=False,
#     affine_ckpt_path=None,            # 跟第一次同一 output_path + graph_list 会命中默认路径
#     strict_affine_ckpt_match=True,     # 建议开着，防止用错ckpt
# )
