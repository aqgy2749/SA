...
output_path = f'{work_dir}/output_castProj4'       ####################
...
# # 读入你前面保存的 labels（比如 cell_label_dict_k20.pt）
# cell_label_dict = torch.load(f"{output_path}/cell_label_dict_k20.pt", map_location="cpu")

# col = "kmeans20"
# if col not in sdata.obs.columns:
#     sdata.obs[col] = np.nan   # 普通列（object）

# for sample, labels in cell_label_dict.items():
#     mask = sdata.obs["sample"].astype(str) == str(sample)
#     labels = np.asarray(labels)

#     assert mask.sum() == labels.shape[0]

#     sdata.obs.loc[mask, col] = [f"K{int(x)}" for x in labels]


# save_path = "/p1/data/jtian/SA/05_CAST/output_castProj4"  
# sdata.write_h5ad(f'{save_path}/demo1_castProj4.h5ad')


sdata = sc.read_h5ad(f'{output_path}/demo1_castProj4.h5ad')
...
#     }
k = 20
palette = (sc.pl.palettes.default_102  # scanpy 自带很多颜色
           if k <= len(sc.pl.palettes.default_102)
           else sc.pl.palettes.vega_20_scanpy)

color_dict = {f"K{i}": palette[i] for i in range(k)}
for rep in graph_list.keys():
    ...
    sdata_refs[rep],list_ts[rep] = CAST.CAST_PROJECT(
        ...
        source_sample_ctype_col = "kmeans20",
