...
output_path = f'{work_dir}/output_cast11'      ############
...
if not all_X_t:
    ...
    # print(adata.X[:5])
    # print(np.min(adata.X), np.max(adata.X))
    OUTPUT_H5AD = f'{output_path}/demo1_cast11.h5ad'                    ########################
    adata.write_h5ad(OUTPUT_H5AD)
    print(f"\n✅ 成功保存 AnnData 文件到 {OUTPUT_H5AD}")

###########################CAST.MARK##########################################
adata = ad.read_h5ad(f'{output_path}/demo1_cast11.h5ad')                #########################
...
cell_label_all = kmeans_plot_multiple(embed_dict,samples,coords_raw,'demo1',output_path,k=20,dot_size = 10,minibatch=False)

# ✅ 1) 把全局 labels 按 samples 顺序切开，变成每个样本自己的 labels
cell_label_dict = {}
offset = 0
for s in samples:  # 注意：必须和你传给 kmeans_plot_multiple 的 samples 顺序一致
    n = coords_raw[s].shape[0]
    cell_label_dict[str(s)] = cell_label_all[offset: offset + n]
    offset += n
# ✅ 2) 保存到 output_path（或你想放的任意目录）
torch.save(cell_label_dict, os.path.join(output_path, "cell_label_dict_k20.pt"))#STACK 运行时的 output_path 必须是同一个目录
print("saved:", os.path.join(output_path, "cell_label_dict_k20.pt"))
...
###########################CAST.MARK##########################################
work_dir = '/p1/data/jtian/SA/05_CAST/'
output_path = f'{work_dir}/output_cast11_houbanduan'      ############
os.makedirs(output_path, exist_ok=True)
adata = ad.read_h5ad("/p1/data/jtian/SA/05_CAST/output_cast11/demo1_cast9.h5ad")                #########################
adata.layers['norm_1e4'] = adata.X 
samples = np.unique(adata.obs['sample'])
coords_raw = {sample_t: np.array(adata.obs[['x','y']])[adata.obs['sample'] == sample_t] for sample_t in samples}
for s in samples:
