...
#     coords_raw, embed_dict, output_path, graph_list,mid_visual=True
#     params_dist=params_dist,
#     load_affine_ckpt=False,
#     save_affine_ckpt=True,
#     affine_ckpt_path=None,  # 用默认路径即可
# )
graph_list = ['30','15']
params_dist = CAST.reg_params(dataname = graph_list[0], 
                            gpu = 0 if torch.cuda.is_available() else -1,
                            ...
                            mesh_weight = [None])
params_dist.alpha_basis = torch.Tensor([1/1000,1/1000,1/50,5,5]).reshape(5,1).to(params_dist.device)
coords_final = CAST.CAST_STACK(coords_raw, embed_dict, output_path, graph_list,mid_visual=True
    ,params_dist=params_dist,
    load_affine_ckpt=False,
    save_affine_ckpt=True,
