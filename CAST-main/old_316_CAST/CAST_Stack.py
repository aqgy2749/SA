...
def register_result(coords_q, coords_r, cov_anchor_t, bleeding, embed_stack, output_path, k=8, prefix='test', scale_t=1, index_list=None):
    ...
    # ====== panel 2b: overlay (query + ref in one axis) ======
    plt.figure(figsize=[10, 12])
    plt.rcParams.update({'font.size': 10, 'axes.titlesize': 20, 'pdf.fonttype': 42})
    # query points
    col_q = coords_q[idx_q, 0]
    row_q = coords_q[idx_q, 1]
    cell_type_q = cell_label[:coords_q[idx_q].shape[0]]
    for i in set(cell_type_q):
        plt.scatter(np.array(col_q)[cell_type_q == i],
                    np.array(row_q)[cell_type_q == i],
                    s=12, edgecolors='none', alpha=0.5, rasterized=True,
                    c=np.array(cluster_pl)[cell_type_q[cell_type_q == i]],
                    label=f"Q-{i}")   # 也可以只 label=str(i)
    # ref points（叠在同一张图上）
    ...
    for i in set(cell_type_r):
        plt.scatter(np.array(col_r)[cell_type_r == i],
                    ...
                    s=12, edgecolors='none', alpha=0.5, rasterized=True,
                    c=np.array(cluster_pl)[cell_type_r[cell_type_r == i]],
                    label=f"R-{i}")
    plt.axis('equal')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    ...
    plt.colorbar()
    plt.savefig(f'{output_path}/{prefix}_Results_3.pdf',dpi = 300)
# def register_result(coords_q, coords_r, cov_anchor_t, bleeding, embed_stack, output_path,
#                     k=8, prefix='test', scale_t=1, index_list=None, cell_label=None):
...
#     plt.subplot(1, 2, 1)
#     t_score = J_cal(torch.from_numpy(coords_q[idx_q]), torch.from_numpy(coords_r[idx_r]), cov_anchor_t, bleeding)
#     plt.scatter(coords_q[idx_q, 0], coords_q[idx_q, 1],
#                 c=1 - t_score, cmap='vlag', vmin=-1, vmax=1,
#                 s=15, edgecolors='none', alpha=0.5, rasterized=True)
#     add_scale_bar(200, '200 µm')

#     plt.subplot(1, 2, 2)
#     plt.scatter(coords_q[0, 0], coords_q[0, 1],
#                 c=[0], cmap='vlag', vmin=-1, vmax=1,
#                 s=15, alpha=0.5)
#     plt.axis('off')
#     plt.colorbar()
#     plt.savefig(f'{output_path}/{prefix}_Results_3.pdf', dpi=300)
