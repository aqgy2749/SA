...
def Harmony_integration(
    ...
    # 用指定 scaled_layer 作为 X（你原来就这么做，保留）
    sdata_inte.X = sdata_inte.layers[scaled_layer].copy()
    ...
    sc.tl.pca(
        sdata_inte,
        use_highly_variable=use_highly_variable_t,
        svd_solver='full',
        n_comps=n_components
    )

    # ============================
    # ✅ CHANGED: 不再使用 scanpy wrapper
    # 原因：你的 harmonypy 在 “PyTorch on cuda” 模式下输出可能是 cuda tensor，
    # scanpy wrapper 直接写入 obsm 会被 anndata 识别成 (50,) 一维，导致 shape 报错。
    # 这里改为：直接调用 harmonypy，并强制把输出转成 CPU numpy 2D，再写入 obsm。
    # ============================
    print('Running Harmony integration (manual harmonypy -> cpu numpy):')

    import numpy as np
    import harmonypy

    # 🧪 DEBUG：确认输入 PCA embedding 形状
    X_pca = np.asarray(sdata_inte.obsm["X_pca"])
    print("DEBUG before harmony: X_pca shape =", X_pca.shape)

    # harmonypy 期望 float64
    X = X_pca.astype(np.float64, copy=False)

    # 运行 Harmony
    ho = harmonypy.run_harmony(X, sdata_inte.obs, batch_key)

    # 取输出（可能是 numpy，也可能是 torch tensor/cuda）
    Z = ho.Z_corr  # 常见形状：(n_pcs, n_obs) 或 (n_obs, n_pcs)

    # ✅ CHANGED：强制转 CPU numpy
    if hasattr(Z, "detach"):   # torch.Tensor
        Z = Z.detach()
    if hasattr(Z, "cpu"):      # cuda -> cpu
        Z = Z.cpu()
    if hasattr(Z, "numpy"):    # tensor -> numpy
        Z = Z.numpy()

    Z = np.asarray(Z)

    # ✅ CHANGED：自动判断是否需要转置，保证写入形状为 (n_obs, n_pcs)
    n_obs = sdata_inte.n_obs
    if Z.ndim != 2:
        raise ValueError(f"Harmony output ho.Z_corr should be 2D, got shape {Z.shape}")
    ...
    if Z.shape[0] == n_obs:
        ...
        raise ValueError(
            f"Unexpected Harmony output shape {Z.shape}; n_obs={n_obs}; X_pca={X.shape}"
        )

    sdata_inte.obsm["X_pca_harmony"] = X_har
    print("DEBUG X_pca_harmony shape:", sdata_inte.obsm["X_pca_harmony"].shape)
    # ============================
    ...
    if ifplot == True:
        ...
        if source_sample_ctype_col is not None:
            sc.pl.umap(sdata_inte, color=[source_sample_ctype_col], size=10, save=f'_har_{umap_n_pcs}pcs_ctype.pdf')
