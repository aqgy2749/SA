...
from .utils import *
from .visualize import *
from .models.model_GCNII import Args, CCA_SSG_MULTIENC   # ✅ 注意这里导入新类

def CAST_MARK(coords_raw_t, exp_dict_t, output_path_t, task_name_t=None, gpu_t=None,
              args=None, epoch_t=None, if_plot=True, graph_strategy='convex'):
    ...
    print(f'Constructing delaunay graphs for {len(samples)} samples...')
    for sample_t in samples:
        graph_dgl_t = delaunay_dgl(sample_t, coords_raw_t[sample_t], output_path_t,
                                   if_plot=if_plot, strategy_t=graph_strategy).to(device)
        feat_torch_t = torch.tensor(exp_dict_t[sample_t], dtype=torch.float32, device=device)
        inputs.append((sample_t, graph_dgl_t, feat_torch_t))

    if args is None:
        args = Args(
            dataname=task_name_t,
            gpu=gpu_t,
            epochs=400,
            lr1=1e-3,
            wd1=0,
    ...
    # ===================== ✅ NEW: per-sample input dims =====================
    in_dim_map = {name: feat.size(-1) for name, _, feat in inputs}
    print("[CAST_MARK] in_dim_map:", in_dim_map)

    # ✅ 初始化多 encoder + 共享 backbone 的模型
    model = CCA_SSG_MULTIENC(
        in_dim_map=in_dim_map,
    ...
    embed_dict, loss_log, model = train_seq(
        ...
        args=args,
        dump_epoch_list=[],
        out_prefix=f'{output_path_t}/{task_name_t}_seq_train',
        model=model
    )
...
def CAST_STACK(
    ...
    # ===== NEW: affine checkpoint controls =====
    affine_ckpt_path=None,
    save_affine_ckpt=True,
    load_affine_ckpt=False,
    strict_affine_ckpt_match=True,  # 若ckpt里query/ref不匹配，是否强制重跑Affine
):
    """
    改动点：
    1) 支持保存/加载 Affine 结果（theta_r1/theta_r2），下次可跳过 prelocate + Affine_GD，直接跑 B-Spline
    2) 当 load_affine_ckpt=True 且 ckpt 存在并匹配 graph_list 时，直接复用 affine 结果
    """
    ...
    coords_raw, result_log["ref_rescale_factor"] = rescale_coords(
        coords_raw, graph_list, rescale=rescale
    )
    ...
    if params_dist.alpha_basis == []:
        params_dist.alpha_basis = (
            torch.Tensor([1 / 3000, 1 / 3000, 1 / 100, 5, 5])
            .reshape(5, 1)
    ...
    if corr_q_r is None:
        if if_embed_sub:
            ...
            corr_q_r = corr_dist(
                embed_dict[query_sample].cpu(), embed_dict[ref_sample].cpu()
            )
    else:
        corr_q_r = corr_q_r
    ...
    plot_mid(
        coords_raw[query_sample], coords_raw[ref_sample], output_path, f"{prefix_t}_raw"
    )

    # ===== NEW: default affine ckpt path =====
    if affine_ckpt_path is None:
        affine_ckpt_path = os.path.join(output_path, f"{prefix_t}_affine_ckpt.pt")

    # ===== Initialize the coordinates and tensor =====
    corr_q_r_t = torch.Tensor(corr_q_r).to(params_dist.device)
    params_dist.mean_q = coords_raw[query_sample].mean(0)
    params_dist.mean_r = coords_raw[ref_sample].mean(0)
    coords_query = torch.Tensor(coords_minus_mean(coords_raw[query_sample])).to(
        params_dist.device
    )
    coords_ref = torch.Tensor(coords_minus_mean(coords_raw[ref_sample])).to(
        params_dist.device
    )

    # ======================================================================
    # NEW: load affine (theta_r1/theta_r2) and skip prelocate + Affine_GD
    # ======================================================================
    affine_loaded = False
    if load_affine_ckpt and os.path.exists(affine_ckpt_path):
        ckpt = torch.load(affine_ckpt_path, map_location="cpu")

        # 兼容两种保存格式：dict(theta_r1/theta_r2) 或保存整个对象
        if isinstance(ckpt, dict) and ("theta_r1" in ckpt) and ("theta_r2" in ckpt):
            ckpt_q = ckpt.get("query_sample", None)
            ckpt_r = ckpt.get("ref_sample", None)

            # 可选：严格检查 query/ref 是否一致
            if strict_affine_ckpt_match and (ckpt_q is not None) and (ckpt_r is not None):
                if (ckpt_q != query_sample) or (ckpt_r != ref_sample):
                    affine_loaded = False
                else:
                    theta_r1 = ckpt["theta_r1"]
                    theta_r2 = ckpt["theta_r2"]
                    affine_loaded = True
            else:
                theta_r1 = ckpt["theta_r1"]
                theta_r2 = ckpt["theta_r2"]
            ...
            # 假设保存的是 params_dist 或类似对象
            # 这种方式无法强校验 query/ref，只能直接用
            theta_r1 = ckpt.theta_r1
            theta_r2 = ckpt.theta_r2
            affine_loaded = True

        if affine_loaded:
            params_dist.theta_r1 = theta_r1.to(params_dist.device)
            params_dist.theta_r2 = theta_r2.to(params_dist.device)

            coords_query_r1 = affine_trans_t(params_dist.theta_r1, coords_query)
            coords_query_r2 = affine_trans_t(params_dist.theta_r2, coords_query_r1)

            result_log["affine_loaded_from_ckpt"] = True
            result_log["affine_ckpt_path"] = affine_ckpt_path
            result_log["affine_J"] = None
            result_log["affine_it_theta"] = None
            result_log["affine_coords_log"] = None
            result_log["coords_ref"] = coords_ref

    # ======================================================================
    # If not loaded, run original prelocate + Affine_GD, then save ckpt
    # ======================================================================
    if not affine_loaded:
        ...
        result_log["affine_loaded_from_ckpt"] = False
        result_log["affine_J"] = similarity_score
        result_log["affine_it_theta"] = it_theta
        result_log["affine_coords_log"] = coords_log
        result_log["coords_ref"] = coords_ref

        # ---- NEW: save affine ckpt right after affine ----
        if save_affine_ckpt:
            torch.save(
                {
                    "query_sample": query_sample,
                    "ref_sample": ref_sample,
                    "theta_r1": params_dist.theta_r1.detach().cpu(),
                    "theta_r2": params_dist.theta_r2.detach().cpu(),
                },
                affine_ckpt_path,
            )

    # ===== Affine results (可视化/登记) =====
    # 注意：如果 affine_loaded=True，没有 it_theta/similarity_score，这里就跳过 affine_reg_params
    if (not affine_loaded) and (result_log.get("affine_it_theta", None) is not None):
        affine_reg_params(
            [i.cpu().numpy() for i in result_log["affine_it_theta"]],
            result_log["affine_J"],
            params_dist.iterations,
            output_path,
            prefix=prefix_t,
        )
    ...
    if if_embed_sub:
        ...
        embed_stack_t = np.row_stack(
            (
                embed_dict[query_sample].cpu().detach().numpy(),
                embed_dict[ref_sample].cpu().detach().numpy(),
            )
        )

    # 这里 coords_query_r2 一定存在（要么加载得到，要么训练得到）
    register_result(
        coords_query_r2.cpu().detach().numpy(),
        coords_ref.cpu().detach().numpy(),
        max_minus_value_t(corr_q_r_t).cpu(),
        params_dist.bleeding,
        embed_stack_t,
        output_path,
        k=20,
    ...
    if params_dist.iterations_bs[round_t] != 0:
        padding_rate = params_dist.PaddingRate_bs  # by default, 0
        coords_query_r2_min = coords_query_r2.min(0)[0]
        coords_query_r2_tmp = coords_minus_min_t(coords_query_r2)
        max_xy_tmp = coords_query_r2_tmp.max(0)[0]
        adj_min_qr2 = coords_query_r2_min - max_xy_tmp * padding_rate
        setattr(
            params_dist,
            "img_size_bs",
            [(max_xy_tmp * (1 + padding_rate * 2)).cpu()],
        )
        params_dist.min_qr2 = [adj_min_qr2]
        ...
        register_result(
            ...
            + str(round_t)
            + "_BSpine_"
            + str(params_dist.iterations_bs[round_t]),
            index_list=[sub_node_idxs[k_t] for k_t in graph_list],
        )
    ...
    coords_final[query_sample] = coords_q_final.cpu() / result_log["ref_rescale_factor"]
    coords_final[ref_sample] = coords_raw[ref_sample] / result_log["ref_rescale_factor"]
...
#     """
#     改动点（相对你这版）：
#     ✅ 1) 从 output_path/cell_label_dict_k20.pt 读取 MARK 的第一次 KMeans 结果
#     ✅ 2) 按 query+ref 的顺序拼成 cell_label_stack_t，并传给 register_result(..., cell_label=...)
#        -> STACK 画图不再重新做 KMeans，而是用你保存的 labels 上色
#     """
...
#     # ======================================================================
#     # load affine (theta_r1/theta_r2) and skip prelocate + Affine_GD
#     # ======================================================================
#     affine_loaded = False
#     if load_affine_ckpt and os.path.exists(affine_ckpt_path):
#         ckpt = torch.load(affine_ckpt_path, map_location="cpu")
...
#     # # ======================================================================
#     # # ✅ NEW：从 MARK 保存的 cell_label_dict_k20.pt 读取 labels，并拼成与 embed_stack_t 一致的顺序
#     # # ======================================================================
#     # cell_label_stack_t = None
#     # label_path = os.path.join(output_path, "cell_label_dict_k20.pt")
#     # if os.path.exists(label_path):
#     #     d = torch.load(label_path, map_location="cpu")
#     #     d = {str(k): np.asarray(v) for k, v in d.items()}

#     #     if if_embed_sub:
#     #         q_lab = d[str(query_sample)][sub_node_idxs[query_sample]]
#     #         r_lab = d[str(ref_sample)][sub_node_idxs[ref_sample]]
#     #     else:
#     #         q_lab = d[str(query_sample)]
#     #         r_lab = d[str(ref_sample)]

#     #     cell_label_stack_t = np.concatenate([q_lab, r_lab], axis=0)
#     # # ======================================================================
...
#     #     scale_t=1,
#     #     index_list=[sub_node_idxs[k_t] for k_t in graph_list],
#     #     cell_label=cell_label_stack_t,   # ✅ NEW
#     # )
...
#             + str(params_dist.iterations_bs[round_t]),
#             index_list=[sub_node_idxs[k_t] for k_t in graph_list],
#             cell_label=cell_label_stack_t,   # ✅ NEW（同一套 labels 继续用）
#         )
