...
def link_plot_3d(assign_mat, coords_target, coords_source, k, figsize_t = [15,20], sample_n=1000, link_color_mask=None, color_target="#9295CA", color_source='#E66665', color_true = "#999999", color_false = "#999999", remove_background = True):
    ...
    xy_all = np.vstack([coords_target, coords_source])
    minx, miny = xy_all.min(axis=0)
    maxx, maxy = xy_all.max(axis=0)
    # 保持 x/y 同尺度，避免变形
    ...
    if link_color_mask is not None:
        ...
        line_collection = Line3DCollection(segs, colors=color_false, lw=1.5, linestyles='dashed')
    ...
    line_collection = Line3DCollection(segs, colors=color_true, lw=1.5, linestyles='dashed')
    line_collection.set_rasterized(True)
    ax.add_collection(line_collection)
    ...
    ax.scatter(coords_target[:, 0], coords_target[:, 1], 0, c=color_target, s=15, alpha=1.0, depthshade=False, linewidths=0)
    ax.scatter(coords_source[:, 0], coords_source[:, 1], 1, c=color_source, s=15, alpha=1.0, depthshade=False, linewidths=0)
