...
# ===== 降采样设置 =====
# 每个样本最多保留多少个 Spot/cell。设为 Inf 可关闭降采样。
max_cells_per_sample <- 5000

# ===== 路径设置 =====
input_dir <- "/p2/zulab/jtian/data/SA/05_CAST/input/LipidsIntensity"
output_dir <- "/p2/zulab/jtian/data/SA/R-Integration/Integration1_downsample_5000"
...
process_sample <- function(file, sample, condition, time) {
  ...
  if (is.finite(max_cells_per_sample) && n_cells_before > max_cells_per_sample) {
    keep_cells <- sort(sample(seq_len(n_cells_before), max_cells_per_sample))
    data <- data[, keep_cells, drop = FALSE]
  }
  message(
    sample,
    ": cells before downsample = ", n_cells_before,
    ", after = ", ncol(data),
    ", features = ", nrow(data)
  )
